...
|---|------|--------|-------|---------------|
| 189 | 2026-02-15 | P0.5.1–P0.5.4: Add Playwright E2E test infrastructure — installed `@playwright/test`, created `playwright.config.ts` (single worker, auth-setup project, chromium desktop + iPhone 14 WebKit for @mobile tests, webServer auto-starts dev); created auth fixtures (`auth.setup.ts` pre-login admin + test user with storageState, `auth.ts` custom fixtures for adminPage/userPage, `test-data.ts` dynamic product helpers); wrote 10 test suites with ~75 tests covering all critical flows: homepage/nav (8), product search with debounce (10), product detail/guide viewer (9), auth login/register/guards (10), saved products (7), guide progress (5), chat troubleshooting with intake phases (6), pricing/subscription (7), studio admin panel (8), retailer pages (5); added `e2e` job to CI (depends on build, installs Playwright browsers, runs against Supabase via GitHub Secrets, uploads HTML report always + test-results on failure); added npm scripts `test:e2e`, `test:e2e:ui`, `test:e2e:headed`; updated `.gitignore` for `e2e/.auth/`, `playwright-report/`, `test-results/` | 0 | playwright.config.ts, e2e/fixtures/auth.setup.ts, e2e/fixtures/auth.ts, e2e/fixtures/test-data.ts, e2e/tests/01-homepage-navigation.spec.ts, e2e/tests/02-product-search.spec.ts, e2e/tests/03-product-detail-guide.spec.ts, e2e/tests/04-auth-flow.spec.ts, e2e/tests/05-saved-products.spec.ts, e2e/tests/06-guide-progress.spec.ts, e2e/tests/07-chat-troubleshooting.spec.ts, e2e/tests/08-pricing-subscription.spec.ts, e2e/tests/09-studio-admin.spec.ts, e2e/tests/10-retailer-pages.spec.ts, .github/workflows/ci.yml, package.json, .gitignore, docs/tasks.md |
